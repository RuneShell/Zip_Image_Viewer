...
$(document).ready(function(){
    ...
    $(document).on("click", ".FileList", async function(e){ //using $(".FileList") -> error
        // For Current File
        if(displayedImgSetIndex >= 0){ // not -1(initial val)
            const displayedImgSetDivId = "fileList:" + imgSetList[displayedImgSetIndex][0].type.toString() + ":" + displayedImgSetIndex.toString();
            const displayedImgSetDiv = document.getElementById(displayedImgSetDivId);
            displayedImgSetDiv.style.backgroundColor = "transparent";
        }
        let elementId = $(this).attr('id').split(":");
        DisplayImgSet(elementId[1], elementId[2]);
    });
...
function DisplayImgData(imgSetIndex_){
...
async function DisplayEpubData(bookSetIndex_){ // epub rendering init부분이 많아서 따로 분리해도 될듯.
    console.log("DisplayEpubData", bookSetIndex_);
...
function PreviousImage(){
    ...
    if (isImgSetListLoaded && (clickedElement != undefined)){
        ...
        if (Number(clickedElementId[2]) <= 1){ // 0(add-fitting) or 1(first image)
            PrevImgSet(displayedImgSetIndex);
            return;
        }
        //if display mode is single-page
        if(imgDisplayMode == imgDisplayModes[0] || imgDisplayMode == imgDisplayModes[2]){
    ...
    if (isImgSetListLoaded && (clickedElement != undefined)){
        ...
        if(imgDisplayMode == imgDisplayModes[0] || imgDisplayMode == imgDisplayModes[2]){
            ...
            if (clickedElementId[2] == imgSetList[Number(clickedElementId[1])][1].length){
                NextImgSet(displayedImgSetIndex);
                return;
            }
            ChangeImgPreview(`${clickedElementId[0]}:${clickedElementId[1]}:${Number(clickedElementId[2]) + 1}`);
        }
...
function PrevImgSet(currentImgSetIndex){
    if(currentImgSetIndex === 0){
        console.log("Can't display previous ImgSet: You're already on first ImgSet.");
        return;
    }

    // For Current File
    const displayedImgSetDivId = "fileList:" + imgSetList[currentImgSetIndex][0].type.toString() + ":" + currentImgSetIndex.toString();
    const displayedImgSetDiv = document.getElementById(displayedImgSetDivId);
    displayedImgSetDiv.style.backgroundColor = "transparent";

    // For Previous File
    currentImgSetIndex--;
    DisplayImgSet(imgSetList[currentImgSetIndex][0].type, currentImgSetIndex);
}
function NextImgSet(currentImgSetIndex){
    if(currentImgSetIndex === imgSetList.length-1){
        console.log("Can't display next ImgSet: You're already on last ImgSet.");
        return;
    }
...
async function DisplayImgSet(imgSetType, imgSetIndex){
    if(imgSetType == 0 || imgSetType== 3){ //img || dir
        DisplayImgData(imgSetIndex);
    }else if(imgSetType == 2){ //epub
        await DisplayEpubData(imgSetIndex).then((r)=>{rendition = r}); // First time, no rendition.
    }

    const imgSetType_backgroundColor_map = {
        0: _ImgBackgroundColor,
        1: _ZipBackgroundColor,
        2: _EpubBackgroundColor,
        3: _ImgBackgroundColor
    }

    // For Changed File CSS
    const displayedImgSetDivId = "fileList:" + imgSetList[imgSetIndex][0].type.toString() + ":" + imgSetIndex.toString();
    const displayedImgSetDiv = document.getElementById(displayedImgSetDivId);
    displayedImgSetDiv.style.backgroundColor = imgSetType_backgroundColor_map[imgSetType];
    displayedImgSetIndex = Number(imgSetIndex);
}
